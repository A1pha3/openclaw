...
如果你正在构建操作员或节点客户端，请使用[网关协议](/gateway/protocol)。

**注意：** 当前 OpenClaw 构建不再随附 TCP 桥接监听器；本文档保留以供历史参考。旧的 `bridge.*` 配置键不再是配置模式的一部分。

## 为什么有两个协议

- **安全边界**：桥接暴露一个小的允许列表，而不是完整的网关 API 表面。
- **配对 + 节点身份**：节点准入由网关拥有，并绑定到每个节点的令牌。
- **发现用户体验**：节点可以通过 LAN 上的 Bonjour 发现网关，或直接在 tailnet 上连接。
...
- `req` / `res`：作用域网关 RPC（聊天、会话、配置、健康、voicewake、skills.bins）
...
节点可以发出 `exec.finished` 或 `exec.denied` 事件以呈现 system.run 活动。这些映射到网关中的系统事件。（旧版节点可能仍然发出 `exec.started`。）
...
- Bonjour **不会**跨网络；在需要时使用手动主机/端口或广域 DNS-SD。

## 版本控制

桥接当前是**隐式 v1**（没有 min/max 协商）。预期向后兼容；在任何破坏性更改之前添加桥接协议版本字段。
