...
以下示例与当前配置模式对齐。详尽的参考和每个字段的注释，请参阅[配置](/gateway/configuration)。
...
保存到 `~/.openclaw/openclaw.json`，你就可以从该号码 DM 机器人。
...
```json5
{
  ...
  // 认证配置文件元数据（机密位于 auth-profiles.json）
  auth: {
    profiles: {
...
- 提供商 ID 不同（电话号码、用户 ID、通道 ID）。使用提供商文档确认格式。
