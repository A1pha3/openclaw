...
| 阶段 | 文档 | 预计时间 | 核心收获 |
|------|------|----------|----------|
| 第一步 | [新手上路](./start/getting-started.md) | 30 分钟 | 完成安装并发送第一条消息 |
| 第二步 | [向导模式详解](./start/wizard.md) | 20 分钟 | 理解配置流程和背后的原理 |
| 第三步 | [安装指南](./install/index.md) | 15 分钟 | 掌握多种安装方式和环境准备 |
| 第四步 | [系统架构](./concepts/architecture.md) | 30 分钟 | 理解整体架构和设计哲学 |
| 第五步 | [渠道配置](./channels/index.md) | 45 分钟 | 连接你需要的消息渠道 |
...
| 阶段 | 文档 | 预计时间 | 核心收获 |
|------|------|----------|----------|
| 第一步 | [开发者概述](./developer/index.md) | 15 分钟 | 了解开发环境和贡献流程 |
| 第二步 | [项目结构](./developer/project-structure.md) | 30 分钟 | 理解代码组织和模块划分 |
| 第三步 | [插件开发](./developer/plugin-development.md) | 2 小时 | 开发自定义渠道或功能 |
| 第四步 | [测试指南](./developer/testing.md) | 1 小时 | 掌握测试方法和最佳实践 |
| 第五步 | [贡献指南](./developer/contributing.md) | 30 分钟 | 了解代码规范和提交流程 |
...
| 阶段 | 文档 | 预计时间 | 核心收获 |
|------|------|----------|----------|
| 第一步 | [运维概述](./operations/index.md) | 20 分钟 | 了解生产部署的最佳实践 |
| 第二步 | [部署指南](./operations/deployment.md) | 1 小时 | 掌握生产环境的部署流程 |
| 第三步 | [监控与日志](./operations/monitoring.md) | 45 分钟 | 建立完善的监控体系 |
| 第四步 | [故障排除](./operations/troubleshooting.md) | 1 小时 | 掌握常见问题的诊断和修复 |
| 第五步 | [安全指南](./gateway/security/index.md) | 1 小时 | 确保系统的安全性 |
...
[新手上路](./start/getting-started.md)——这是每位新用户必读的入门指南，从零到完成第一次 AI 对话的完整教程。它涵盖环境准备、安装步骤、初始配置以及如何发送第一条消息。文档中包含大量的截图和步骤说明，即使你完全不熟悉命令行也能顺利完成安装。完成本指南后，你将拥有一个可以正常运行的 OpenClaw 环境，并能够通过选定的渠道与 AI 助手进行对话。

[快速入门](./start/quick-start.md)——如果你已经有一定的技术背景，想要快速体验 OpenClaw 的核心功能，这个 5 分钟快速上手指南是最佳选择。它以最精简的步骤展示 OpenClaw 的核心工作流程，让你在最短时间内理解系统的基本概念。

[向导模式详解](./start/wizard.md)——OpenClaw 提供了交互式的配置向导，帮助用户完成初始设置。这个文档详细解释了向导模式的每一步，以及每一步背后的设计原理。理解向导模式有助于你在需要手动配置时能够更好地理解各个选项的含义。

[安装指南](./start/installation.md)——针对不同操作系统和环境提供详细的安装步骤，包括 macOS、Linux、Windows（WSL2）以及 Docker 容器化部署。每种安装方式都有完整的依赖说明、前置条件和常见问题的解决方案。

### 第二部分：核心概念

**适用场景**：想要深入理解 OpenClaw 的工作原理

[系统架构](./concepts/architecture.md)——这是理解 OpenClaw 的核心文档，详细介绍了系统的整体架构设计。它解释了 Gateway（网关）、代理（Agent）、渠道（Channel）、工具（Tool）之间的关系，以及消息在系统中的完整流转路径。理解架构设计对于后续的高级配置和问题诊断至关重要。

[Gateway 网关](./concepts/gateway.md)——Gateway 是 OpenClaw 的核心控制中心，负责消息路由、认证管理、渠道连接和代理协调。这个文档深入介绍了 Gateway 的设计原理、配置选项和运维最佳实践。

[AI 代理](./concepts/agents.md)——AI 代理是实际处理用户请求的组件。文档介绍了代理的工作原理、提示词工程、上下文管理和会话机制。你将学习如何配置和定制 AI 代理的行为，使其更好地适应你的需求。

[会话管理](./concepts/sessions.md)——会话是代理与用户之间的一次对话交互。这个文档介绍了会话的生命周期管理、上下文机制和状态保持，帮助你理解 AI 如何在对话中保持连贯性。

[消息路由](./concepts/routing.md)——当消息从多个渠道涌入时，路由规则决定了消息如何被处理和转发。这个文档详细介绍了路由配置、优先级规则和自定义路由逻辑。
...
[渠道概述](./channels/index.md)——提供了所有支持渠道的对比表，帮助你根据需求选择合适的渠道。每个渠道的功能特性、配置复杂度、适用场景都有详细说明。

[Telegram](./channels/telegram.md)——Telegram 是最受欢迎的渠道之一，其 Bot API 成熟稳定，配置相对简单。这个文档详细介绍了 Bot Token 的获取、Webhook 配置和群组管理等功能。

[WhatsApp](./channels/whatsapp.md)——WhatsApp 是全球使用最广泛的消息应用。这个文档介绍了如何使用 Baileys 库连接 WhatsApp，包括设备登录、消息收发和群组处理。

[Discord](./channels/discord.md)——Discord 是社区和团队协作的首选平台。这个文档介绍了如何创建 Discord 应用、配置 Bot 权限以及处理服务器和频道中的消息。

[Slack](./channels/slack.md)——Slack 是企业协作的标准工具。这个文档介绍了 Slack App 的创建、工作空间配置和事件订阅等高级功能。

[iMessage](./channels/imessage.md)——对于 macOS 用户，iMessage 是一个便捷的渠道选择。这个文档介绍了如何配置 iMessage 连接和消息收发。

[Signal](./channels/signal.md)——Signal 以隐私保护著称。这个文档介绍了 signal-cli 的配置和使用方法。

### 第四部分：配置参考

**适用场景**：需要详细配置参数和选项

[配置概述](./config/index.md)——配置系统的入门介绍，帮助你理解 OpenClaw 的配置层次结构和配置文件的组织方式。

[配置参考](./config/reference.md)——完整的配置参数手册，涵盖 Gateway、渠道、代理、工具等所有可配置的选项。每个配置项都有类型说明、默认值和示例。

[配置示例](./config/examples.md)——常见场景的配置模板，你可以直接使用或根据需要修改。示例包括个人使用、团队协作、生产部署等多种场景。

### 第五部分：命令行工具

**适用场景**：通过命令行管理 OpenClaw

[CLI 概述](./cli/index.md)——命令行工具的整体介绍，包括命令结构、帮助系统和常用操作。

[onboard](./cli/onboard.md)——引导配置的 CLI 命令，用于完成初始设置和向导式配置。

[gateway](./cli/gateway.md)——网关管理命令，包括启动、停止、状态查看和配置重载。

[config](./cli/config.md)——配置操作的 CLI 命令，支持查看、修改、导入和导出配置。

[status](./cli/status.md)——状态查看命令，显示当前系统的运行状态和资源使用情况。

[doctor](./cli/doctor.md)——诊断工具，自动检测常见配置问题和潜在风险。

[channels](./cli/channels.md)——渠道管理命令，用于添加、删除、配置和测试各个渠道。

[pairing](./cli/pairing.md)——配对管理命令，处理新设备和新渠道的认证配对。

[message](./cli/message.md)——消息发送命令，支持通过 CLI 直接发送消息。

[agent 和 agents](./cli/agents.md)——代理管理命令，用于创建、配置和删除 AI 代理。

[sessions](./cli/sessions.md)——会话管理命令，查看和操作活动会话。

[memory](./cli/memory.md)——记忆管理命令，管理代理的长期记忆和知识库。

### 第六部分：工具与技能

**适用场景**：扩展 AI 的能力边界

[工具概述](./tools/index.md)——工具系统的整体介绍，理解工具如何增强 AI 的能力。

[浏览器工具](./tools/browser.md)——浏览器自动化工具，让 AI 能够访问网页、提取信息和执行网页操作。

[执行工具](./tools/exec.md)——命令执行工具，允许 AI 运行系统命令和脚本。

[Web 工具](./tools/web.md)——Web 搜索和抓取工具，为 AI 提供互联网访问能力。

[思考模式](./tools/thinking.md)——深度思考工具，让 AI 能够进行复杂的多步推理。

[子代理](./tools/subagents.md)——子代理工具，支持任务分发和并行处理。

### 第七部分：网关运维

**适用场景**：部署和运维 Gateway 服务

[网关概述](./gateway/index.md)——Gateway 系统的入门介绍，理解网关的角色和职责。

[配置详解](./concepts/gateway.md)——Gateway 的详细配置选项和最佳实践。

[远程访问](./gateway/remote.md)——配置从外部网络访问 Gateway 的方法，包括 Tailscale 集成和 SSH 隧道。

[Tailscale 集成](./gateway/tailscale.md)——使用 Tailscale 实现安全组网和远程访问。

[后台服务](./gateway/background-process.md)——将 Gateway 配置为系统服务，确保开机自启动和稳定运行。

[健康检查](./gateway/health.md)——健康检查端点和监控配置。

[日志系统](./gateway/logging.md)——日志配置和管理，帮助问题诊断。

[诊断工具](./gateway/doctor.md)——集成的诊断工具，自动检测和修复常见问题。

### 第八部分：AI 提供商

**适用场景**：配置和使用不同的 AI 模型

[提供商概述](./providers/index.md)——所有支持的 AI 服务提供商的对比和选择指南。

[Anthropic Claude](./providers/anthropic.md)——OpenClaw 最推荐的 AI 提供商，详细介绍配置和使用方法。

[OpenAI GPT](./providers/openai.md)——GPT 系列的配置和使用，包括 API Key 配置和模型选择。

[OpenRouter](./providers/openrouter.md)——多模型聚合平台，统一接口访问多个 AI 模型。

[Moonshot Kimi](./providers/moonshot.md)——国产大模型，中文优化和长文本处理能力强。

[GLM](./providers/glm.md)——智谱 AI 的 GLM 系列模型配置。

[MiniMax](./providers/minimax.md)——MiniMax 模型配置和优化。

[Ollama](./providers/ollama.md)——本地模型部署方案，保护隐私和降低成本。

### 第九部分：部署平台

**适用场景**：在不同操作系统和云平台上运行 OpenClaw

[macOS](./platforms/macos.md)——macOS 平台的完整指南，包括原生应用、菜单栏集成和 iMessage 支持。

[Linux](./platforms/linux.md)——Linux 服务器部署指南，推荐 Ubuntu 和其他主流发行版。

[Windows WSL2](./platforms/windows.md)——Windows 用户通过 WSL2 运行 OpenClaw 的完整指南。

[iOS](./platforms/ios.md)——iOS 移动节点配置，使用 iOS 设备作为 AI 的眼睛和耳朵。

[Android](./platforms/android.md)——Android 移动节点配置，类似的移动设备集成。

[Docker](./install/docker.md)——容器化部署方案，便于移植和扩展。

[Fly.io](./platforms/fly.md)——边缘部署方案，全球低延迟访问。

### 第十部分：移动节点

**适用场景**：利用移动设备的硬件能力

[节点概述](./nodes/index.md)——移动设备节点系统的介绍，理解节点如何扩展 AI 的能力。

[音频节点](./nodes/audio.md)——语音输入和音频处理，让 AI 能够听和说。

[相机节点](./nodes/camera.md)——图像捕捉和视觉理解，赋予 AI 视觉能力。

[位置服务](./nodes/location-command.md)——GPS 位置信息，让 AI 知道你在哪里。

[语音对话](./nodes/talk.md)——实时语音对话模式，自然的语言交互体验。

[语音唤醒](./nodes/voicewake.md)——语音唤醒功能，免提激活 AI 助手。

### 第十一部分：自动化

**适用场景**：定时任务和事件驱动自动化

[Cron 任务](./automation/cron-jobs.md)——定时执行任务，例如每日报告、定时提醒等。

[Webhook](./automation/webhook.md)——HTTP 触发方式，接收外部系统的通知和事件。

### 第十二部分：Web 界面

**适用场景**：通过浏览器与 OpenClaw 交互

[Web 概述](./web/index.md)——Web 界面的整体介绍。

[仪表盘](./web/dashboard.md)——浏览器中的聊天界面，随时随地与 AI 对话。
...
[常见问题](./help/faq.md)——80% 的问题都能在这里找到答案，建议先查阅。

[故障排除](./help/troubleshooting.md)——系统化的故障诊断流程，引导你逐步排查问题。

[文档搜索](./search.md)——全文搜索文档，快速定位相关内容。
