...
通过 IRC 连接支持 Twitch 聊天。OpenClaw 作为 Twitch 用户（机器人账户）连接，接收和发送频道中的消息。
...
2. 生成凭据：[Twitch Token Generator](https://twitchtokengenerator.com/)
   - 选择 **Bot Token**
   - 验证范围 `chat:read` 和 `chat:write` 已选中
...
```json5
{
  channels: {
    twitch: {
      ...
      channel: "vevisk", // 要加入的 Twitch 频道（必需）
...
要自动刷新令牌，在 [Twitch Developer Console](https://dev.twitch.tv/console) 创建你自己的 Twitch 应用程序并添加到配置：
...
使用 `channels.twitch.accounts` 和每个账户的令牌。请参阅 [`gateway/configuration`](/gateway/configuration) 了解共享模式。
...
默认情况下，`requireMention` 是 `true`。要禁用并回复所有消息：
