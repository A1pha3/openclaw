...
Nextcloud Talk 作为插件提供，不与核心安装捆绑。
...
如果在配置/引导过程中选择 Nextcloud Talk 并检测到 git 检出，OpenClaw 将自动提供本地安装路径。
...
- Webhook URL 必须可由 Gateway 访问；如果在代理后端，设置 `webhookPublicUrl`。
- 媒体上传不受机器人 API 支持；媒体作为 URL 发送。
- Webhook 载荷无法区分私信与房间；设置 `apiUser` + `apiPassword` 以启用房间类型查找（否则私信被视为房间）。
...
- `channels.nextcloud-talk.chunkMode`：`length`（默认）或 `newline`（在长度分块之前在空行/段落边界处分隔）。
- `channels.nextcloud-talk.blockStreaming`：为此渠道禁用块流式传输。
