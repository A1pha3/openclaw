...
OpenClaw 可以将 Perplexity Sonar 用于 `web_search` 工具。你可以通过 Perplexity 的直接 API 或通过 OpenRouter 连接。
...
如果同时设置了 `PERPLEXITY_API_KEY` 和 `OPENROUTER_API_KEY`，设置 `tools.web.search.perplexity.baseUrl`（或 `tools.web.search.perplexity.apiKey`）以消除歧义。
