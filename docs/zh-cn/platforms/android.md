---
summary: "Android 应用（节点）：连接操作手册 + Canvas/聊天/相机"
read_when:
  - 配对或重新连接 Android 节点
  - 调试 Android 网关发现或认证
  - 验证跨客户端聊天历史一致性
title: "Android 应用"
---

# Android 应用（节点）

## 支持概况

- 角色：伴侣节点应用（Android 不托管网关）。
- 需要网关：是（在 macOS、Linux 或 Windows via WSL2 上运行）。
- 安装：[快速开始](/zh-cn/start/getting-started) + [配对](/zh-cn/gateway/pairing)。
- 网关：[操作手册](/zh-cn/gateway) + [配置](/zh-cn/gateway/configuration)。
  - 协议：[网关协议](/zh-cn/gateway/protocol)（节点 + 控制平面）。

## 系统控制

系统控制（launchd/systemd）在网关主机上。参见 [网关](/zh-cn/gateway)。

## 连接操作手册

Android 节点应用 ⇄ (mDNS/NSD + WebSocket) ⇄ **网关**

Android 直接连接到网关 WebSocket（默认 `ws://<host>:18789`）并使用网关拥有的配对。

### 前置条件

- 你可以在"主"机器上运行网关。
- Android 设备/模拟器可以访问网关 WebSocket：
  - 同一局域网使用 mDNS/NSD，**或**
  - 同一 Tailscale tailnet 使用 Wide-Area Bonjour / unicast DNS-SD（见下文），**或**
  - 手动网关主机/端口（备用方案）
- 你可以在网关机器上（或通过 SSH）运行 CLI（`openclaw`）。

### 1) 启动网关

```bash
openclaw gateway --port 18789 --verbose
```

确认日志中看到类似：

- `listening on ws://0.0.0.0:18789`

对于仅 tailnet 的设置（推荐用于跨地域），将网关绑定到 tailnet IP：

- 在网关主机的 `~/.openclaw/openclaw.json` 中设置 `gateway.bind: "tailnet"`。
- 重启网关 / macOS 菜单栏应用。

### 2) 验证发现（可选）

从网关机器：

```bash
dns-sd -B _openclaw-gw._tcp local.
```

更多调试说明：[Bonjour](/zh-cn/gateway/bonjour)。

#### Tailnet（跨网络）通过 unicast DNS-SD 发现

Android NSD/mDNS 发现无法跨网络。如果你的 Android 节点和网关在不同网络但通过 Tailscale 连接，改用 Wide-Area Bonjour / unicast DNS-SD：

1. 在网关主机上设置 DNS-SD 区域（例如 `openclaw.internal.`）并发布 `_openclaw-gw._tcp` 记录。
2. 为你选择的域名配置 Tailscale split DNS 指向该 DNS 服务器。

详情和 CoreDNS 配置示例：[Bonjour](/zh-cn/gateway/bonjour)。

### 3) 从 Android 连接

在 Android 应用中：

- 应用通过**前台服务**（持久通知）保持网关连接活跃。
- 打开 **Settings**。
- 在 **Discovered Gateways** 下，选择你的网关并点击 **Connect**。
- 如果 mDNS 被阻止，使用 **Advanced → Manual Gateway**（主机 + 端口）并点击 **Connect (Manual)**。

首次成功配对后，Android 在启动时自动重连：

- 手动端点（如果启用），否则
- 最后发现的网关（尽力而为）。

### 4) 批准配对（CLI）

在网关机器上：

```bash
openclaw nodes pending
openclaw nodes approve <requestId>
```

配对详情：[网关配对](/zh-cn/gateway/pairing)。

### 5) 验证节点已连接

- 通过节点状态：
  ```bash
  openclaw nodes status
  ```
- 通过网关：
  ```bash
  openclaw gateway call node.list --params "{}"
  ```

### 6) 聊天 + 历史

Android 节点的 Chat 面板使用网关的**主会话密钥**（`main`），因此历史和回复与 WebChat 和其他客户端共享：

- 历史：`chat.history`
- 发送：`chat.send`
- 推送更新（尽力而为）：`chat.subscribe` → `event:"chat"`

### 7) Canvas + 相机

#### 网关 Canvas 主机（推荐用于 web 内容）

如果你想让节点显示助手可以在磁盘上编辑的真实 HTML/CSS/JS，将节点指向网关 canvas 主机。

注意：节点使用 `canvasHost.port`（默认 `18793`）上的独立 canvas 主机。

1. 在网关主机上创建 `~/.openclaw/workspace/canvas/index.html`。

2. 将节点导航到它（局域网）：

```bash
openclaw nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18793/__openclaw__/canvas/"}'
```

Tailnet（可选）：如果两个设备都在 Tailscale 上，使用 MagicDNS 名称或 tailnet IP 代替 `.local`，例如 `http://<gateway-magicdns>:18793/__openclaw__/canvas/`。

此服务器将实时重载客户端注入到 HTML 中，并在文件更改时重新加载。
A2UI 主机在 `http://<gateway-host>:18793/__openclaw__/a2ui/`。

Canvas 命令（仅前台）：

- `canvas.eval`、`canvas.snapshot`、`canvas.navigate`（使用 `{"url":""}` 或 `{"url":"/"}` 返回默认脚手架）。`canvas.snapshot` 返回 `{ format, base64 }`（默认 `format="jpeg"`）。
- A2UI：`canvas.a2ui.push`、`canvas.a2ui.reset`（`canvas.a2ui.pushJSONL` 旧别名）

相机命令（仅前台；权限控制）：

- `camera.snap`（jpg）
- `camera.clip`（mp4）

参见 [相机节点](/zh-cn/nodes/camera) 了解参数和 CLI 辅助工具。

## 功能对比

| 功能 | Android 节点 | iOS 节点 | macOS 节点 |
|------|-------------|----------|-----------|
| Canvas | ✅ | ✅ | ✅ |
| 聊天 | ✅ | ✅ | ✅ |
| 相机拍照 | ✅ | ✅ | ✅ |
| 相机录像 | ✅ | ✅ | ✅ |
| 屏幕录制 | ✅ | ✅ | ✅ |
| 语音唤醒 | ✅ | ✅ | ✅ |
| 对话模式 | ✅ | ✅ | ✅ |
| 位置获取 | ✅ | ✅ | ✅ |
| SMS（可选） | ✅ | ❌ | ❌ |
| 系统命令 | ❌ | ❌ | ✅ |
| 托管网关 | ❌ | ❌ | ✅ |

## 常见问题排查

| 问题 | 解决方案 |
|------|----------|
| 发现不到网关 | 确保同一网络或使用手动配置 |
| 配对请求未出现 | 检查 `openclaw nodes pending` |
| 连接频繁断开 | 检查 Android 电池优化设置 |
| Canvas 加载失败 | 确认 canvas 主机端口可达 |
| 相机权限问题 | 在 Android 设置中授予权限 |
| 前台服务被杀 | 将应用加入电池优化白名单 |

## Android 特有设置

### 电池优化

为确保可靠连接，建议：

1. 将 OpenClaw 加入电池优化白名单
2. 允许后台运行
3. 禁用自动睡眠（如果需要持续连接）

### 网络权限

应用需要以下权限：
- 网络访问（用于 WebSocket 连接）
- 本地网络发现（用于 mDNS/NSD）
- 相机（可选，用于拍照/录像）
- 麦克风（可选，用于语音功能）
- 位置（可选，用于位置获取）
