---
summary: "在便宜的 Hetzner VPS 上通过 Docker 全天候运行 OpenClaw Gateway，具有持久状态和内置二进制文件"
read_when:
  - 你想在云 VPS 上全天候运行 OpenClaw（而不是在你的笔记本电脑上）
  - 你希望在自己 VPS 上拥有生产级、始终在线的 Gateway
  - 你想要对持久化、二进制文件和重启行为有完全控制
...
如果你想要"大约 5 美元/月的全天候 OpenClaw"，这是最简单可靠的设置。
Hetzner 价格可能会变化；选择最小的 Debian/Ubuntu VPS，如果遇到内存不足问题再升级。
...
services:
  openclaw-gateway:
    ...
    ports:
      # 建议：在 VPS 上保持 Gateway 仅环回访问；通过 SSH 隧道访问。
