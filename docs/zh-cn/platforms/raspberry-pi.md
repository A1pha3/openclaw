---
summary: "Raspberry Pi 上的 OpenClaw（经济型自托管部署）"
read_when:
...
在 Raspberry Pi 上运行持久、始终在线的 OpenClaw Gateway，一次性成本约 **35-80 美元**（无月费）。
...
可定制安装让您可以直接访问日志和代码 — 便于调试 ARM 特定问题。
...
1. **Gateway 模式：** 本地
2. **身份验证：** 推荐使用 API 密钥（OAuth 在无头 Pi 上可能不稳定）
3. **渠道：** Telegram 最容易开始
4. **守护进程：** 是（systemd）
...
**始终使用 64 位操作系统。** Node.js 和许多现代工具都需要它。检查方法：
...
**不要尝试在 Pi 上运行本地 LLM** — 即使是小模型也太慢。让 Claude/GPT 来处理繁重的工作。
...
| **Pi 4 (4GB)** | ~$55       | $0       | 推荐                     |
| **Pi 5 (4GB)** | ~$60       | $0       | 最佳性能                 |
| **Pi 5 (8GB)** | ~$80       | $0       | 超出需求但面向未来        |
| DigitalOcean   | $0         | $6/月    | $72/年                    |
| Hetzner        | $0         | €3.79/月 | ~$50/年                   |

**盈亏平衡：** 与云 VPS 相比，Pi 在约 6-12 个月内即可收回成本。
...
- [Hetzner 指南](/zh-CN/platforms/hetzner) — Docker 设置
- [Tailscale](/zh-CN/gateway/tailscale) — 远程访问
- [Nodes](/zh-CN/nodes) — 将您的笔记本电脑/手机与 Pi gateway 配对
