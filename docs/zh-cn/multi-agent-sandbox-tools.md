...
title: 多代理沙箱和工具
read_when: "你希望在多代理网关中为每个代理配置沙箱或每个代理的工具允许/拒绝策略。"
status: active
---
...
`setupCommand` 属于 `sandbox.docker`（全局或每个代理），并在创建容器时运行一次。

身份验证是每个代理的：每个代理从自己的 `agentDir` 身份验证存储中读取，位置为：

```
~/.openclaw/agents/<agentId>/agent/auth-profiles.json
```

凭据**不会**在代理之间共享。永远不要跨代理重用 `agentDir`。如果你想共享凭据，将 `auth-profiles.json` 复制到其他代理的 `agentDir` 中。

关于沙箱在运行时的行为，请参阅 [沙箱](/gateway/sandboxing)。关于调试"为什么这个被阻止？"，请参阅 [沙箱 vs 工具策略 vs 提升](/gateway/sandbox-vs-tool-policy-vs-elevated) 和 `openclaw sandbox explain`。
...
每个级别可以进一步限制工具，但不能从早期级别授予回被拒绝的工具。如果设置了 `agents.list[].tools.sandbox.tools`，它会替换该代理的 `tools.sandbox.tools`。如果设置了 `agents.list[].tools.profile`，它会覆盖该代理的 `tools.profile`。提供商工具键接受 `provider`（例如 `google-antigravity`）或 `provider/model`（例如 `openai/gpt-5.2`）。

### 工具组（速记）

工具策略（全局、代理、沙箱）支持 `group:*` 条目，扩展为多个具体工具：
...
- 避免允许列出路由到受限代理的发送者
- 如果你只想要沙箱化执行，全局禁用提升（`tools.elevated.enabled: false`）
...
遗留的 `agent.*` 配置由 `openclaw doctor` 迁移；今后首选 `agents.defaults` + `agents.list`。
...
`agents.defaults.sandbox.mode: "non-main"` 基于 `session.mainKey`（默认 `"main"`），而不是代理 ID。群组/频道会话总是获得自己的密钥，因此它们被视为非主会话并将被打沙箱。如果你希望代理永远不打沙箱，设置 `agents.list[].sandbox.mode: "off"`。
